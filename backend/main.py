"""
Surfcasting Analytics API – v9.1 (Deep Synthesis & Sea Memory Injected)
مبني على النسخة 9.0 مع حقن محرك الذاكرة البحرية والعوامل الخفية وتطوير التقرير
"""
...
@app.get("/health")
def health(): return {"status": "ok", "version": "9.1.0"}
...
# ==================== محرك التجميع الفيزيائي المتقدم (مطور) ====================
def aggregate_physics(all_times, aligned, orient, target_date_obj, sunrise, sunset):
    ...
    # ==========================================
    # (جديد) 1. ذاكرة البحر وحالته الراهنة (الـ 48 ساعة الماضية)
    # ==========================================
    ...
    # ==========================================
    # (جديد) 2. ميكانيكا التيار الجانبي وتحريك الرصاصة
    # ==========================================
    ...
    # ==========================================
    # (جديد) 3. العوامل الخفية (Hidden Killers)
    # ==========================================
    freshwater_risk = "منخفض"
    stratification_risk = "منخفض"
    if past_idx:
        p_prec = aligned.get("precipitation", [])
        if any(i < len(p_prec) for i in past_idx):
            if sum(p_prec[i] for i in past_idx if i < len(p_prec)) > 10.0:
                freshwater_risk = "مرتفع جداً (سيول ومياه عذبة طافية تغطي السطح وتمنع الأكسجين)"
                
        p_swh_arr = aligned.get("swell_wave_height", []); p_ws_arr = aligned.get("wind_speed_10m", [])
        valid_strat = [i for i in past_idx if i < len(p_swh_arr) and i < len(p_ws_arr)]
        if valid_strat:
            avg_p_swh = sum(p_swh_arr[i] for i in valid_strat)/len(valid_strat)
            avg_p_ws = sum(p_ws_arr[i] for i in valid_strat)/len(valid_strat)
    ...
    # ==========================================
    # 4. باقي الحسابات الأصلية (معدلة لربط العوامل الجديدة)
    # ==========================================
    if len(sst) > 1:
    ...
    # ربط مصفوفة الأسماك بالذاكرة والعوامل الخفية
    ...
    # (جديد) تحليل ديناميكيات الضغط
    if press_change < -2.0: pressure_state = "انخفاض حاد ومستمر: الأسماك تدرك العاصفة وتتغذى بشراهة. فرصة ذهبية."
    elif press_change < -0.5: pressure_state = "انخفاض بطيء: نشاط جيد ومستمر."
    elif press_change > 1.5: pressure_state = "ارتفاع حاد: الأسماك تمتلئ هواء وتتوقف عن الأكل."
    else: pressure_state = "مستقر: لا تأثير مباشر، العوامل الأخرى حاكمة."
...
# ==================== بناء سياق التفاعلات (مطور لربط كل الداتا) ====================
def build_context(req, agg, tz_name):
    ...
    # 1. سرد الذاكرة البحرية كأساس لكل شيء
    interactions.append(f"ذاكرة البحر وحالته الراهنة: {agg['sea_memory']}")
    
    # 2. الميكانيكا (التيار الجانبي وانجراف الرصاصة)
    ...
    # 6. تفاعل زوايا الموج مع الشاطئ لكل فترة
    for b in agg["blocks"]:
    ...
    # 7. تفاعل خطر الأعشاب
    if agg["weed_risk"]:
        interactions.append("🚨 خطر قاتل: الأعشاب البحرية (البوسيدونيا) مقتلعة من القاع بسبب طاقة الأمواج الماضية وتتجه نحو الشاطئ. ستغلف الخطافات.")
    ...
    lines = [
        ...
        *interactions,
        ...
        "",
        "المطلوب: اكتب تقريراً تحليلياً مركباً ومفصلاً وطويلاً (ليس سردياً). ابدأ بالنتيجة النهائية (Go/No-Go) ثم فكك الأسباب. اربط كل ظاهرة بتأثيرها الميكانيكي على الخطاف والرصاصة والسمكة. اكتب فقرات طويلة ومترابطة تشرح سلسلة ردود الأفعال (مثلاً: الموج المائل خلق تياراً جانبياً، التيار سيجرف الرصاصة، لذا يجب استخدام رصاصة كذا، والسمكة ستكون في مكان كذا بسبب كذا)."
    ]
    return "\n".join(lines)

# ==================== برومبت التحليل الاستنتاجي المطور (لإصدار تقرير طويل ومترابط) ====================
...
قواعد صارمة:
1. اكتب تقريراً مطولاً ومفصلاً. استخدم فقرات متعددة لشرح كل عامل.
...
3. كل فقرة يجب أن تربط ظاهرة فيزيائية (موج/رياح/ضغط/تيار) بتأثيرها المباشر على سلوك السمكة أو ميكانيكية الصيد (انجراف، ثبات، رؤية).
...
async def call_openrouter(ctx):
    ...
    # تم رفع max_tokens لضمان تقرير طويل ومفصل، ورفع temperature قليلاً لزيادة التفصيل
