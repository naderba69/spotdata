"""
Surfcasting Analytics API – v5.2 (تقرير احترافي حي، تتبع تغيرات الرياح، تحليل الموج)
"""
...
app = FastAPI(title="Surfcasting Analytics", version="5.2.0")
...
def evaluate_spot(times, aligned, orient, sunrise_str, sunset_str, beach_type, target_species=None):
    ...
    if target_species and target_species in SPECIES_PREFERENCES:
        ...
        elif any(w in pref["preferred_wind"] for w in wcls): match+=10
...
TUNISIAN_BEACHES = {
    "بنزرت": [
        ...
        # ... أضف كامل القائمة كما في السابق
    ],
    "نابل": [
        ...
        async with sem:
            try:
                m,w = await asyncio.gather(
                    fetch_with_retry(MARINE_URL, {"latitude":b["lat"],"longitude":b["lon"],
                    ...
                    fetch_with_retry(WEATHER_URL, {"latitude":b["lat"],"longitude":b["lon"],
                        "hourly":"wind_speed_10m,wind_direction_10m,wind_gusts_10m,pressure_msl,temperature_2m,precipitation,weather_code",
                        "daily":"sunrise,sunset","timezone":"Africa/Tunis","start_date":start.isoformat(),"end_date":end.isoformat()})
                )
                ...
                return {"name":b["name"],"score":score,"summary":summ,"type":b["type"]}
            except: return None
...
# ========== التجميع المفصّل مع تحليل الموج وتغير الرياح ==========
def aggregate_physics(all_times, aligned, orient, target_date_obj, sunrise, sunset):
    ...
    for key in ["morning", "afternoon", "night"]:
        ...
        # تحليل مكونات الموج: هل swell هو المسيطر أم wind sea
...
async def generate_report(request: Request, req: ReportRequest):
    ...
    try:
        ...
        marine, weather = await asyncio.gather(
            fetch_with_retry(MARINE_URL, {"latitude":req.latitude,"longitude":req.longitude,
                "hourly":["wave_height","wave_period","wave_direction","swell_wave_height","swell_wave_period","swell_wave_direction","sea_surface_temperature"],
                "timezone":tz_name,"start_date":start.isoformat(),"end_date":end.isoformat()}),
            fetch_with_retry(WEATHER_URL, {"latitude":req.latitude,"longitude":req.longitude,
                "hourly":["wind_speed_10m","wind_direction_10m","wind_gusts_10m","pressure_msl","temperature_2m","precipitation","weather_code"],
                "daily":["sunrise","sunset"],"timezone":tz_name,"start_date":start.isoformat(),"end_date":end.isoformat()})
        )
